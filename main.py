...
def orders():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT products.productId, products.name, products.price, products.image FROM products, orders WHERE products.productId = orders.productId AND orders.userId = ?", (userId, ))
...
def changePassword():
    ...
    if request.method == "POST":
        ...
        conn.close()
        return render_template("changePassword.html", profileData=profileData, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, msg=msg)
    else:
...
def checkout():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT userId FROM users WHERE email = ?", (email, ))
        userId = cur.fetchone()[0]
        cur.execute("INSERT INTO orders SELECT * FROM kart")
        products = cur.fetchall()
        try:
